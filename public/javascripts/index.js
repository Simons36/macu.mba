...
function init() {
	if (!WebTorrent.WEBRTC_SUPPORT) {
		util.error(
			"This browser is unsupported. Please use a browser with WebRTC support."
		);
	}
	...
	if (form) {
		form.addEventListener("submit", function (e) {
			...
			downloadTorrent(
				document
					.querySelector("form input[name=torrentId]")
					.value.trim()
			);
		});
	}
		...
		function (err, res, data) {
			if (err || res.statusCode !== 200) {
				...
				try {
					...
					return cb(
						new Error(
							"Got invalid WebRTC config from server: " + data
						)
					);
				}
...
function downloadTorrentFile(file) {
	util.unsafeLog(
		"Downloading torrent from <strong>" +
...
function onTorrent(torrent) {
	...
	util.log(
		'"' + torrentFileName + '" contains ' + torrent.files.length + " files:"
	);
	...
	torrent.files.forEach(function (file) {
		util.unsafeLog(
			"&nbsp;&nbsp;- " +
				escapeHtml(file.name) +
				" (" +
				escapeHtml(prettierBytes(file.length)) +
				")"
		);
	});
	...
	util.unsafeLog(
		'<a href="/#' +
			escapeHtml(torrent.infoHash) +
			'" onclick="prompt(\'Share this link with anyone you want to download this torrent:\', this.href);return false;">[Share link]</a> ' +
			'<a href="' +
			escapeHtml(torrent.magnetURI) +
			'" target="_blank">[Magnet URI]</a> ' +
			'<a href="' +
			escapeHtml(torrent.torrentFileBlobURL) +
			'" target="_blank" download="' +
			escapeHtml(torrentFileName) +
			'">[Download .torrent]</a>'
	);
	...
	function updateSpeed() {
		...
		util.updateSpeed(
			"<b>Peers:</b> " +
				...
				"% " +
				"<b>Download speed:</b> " +
				prettierBytes(window.client.downloadSpeed) +
				"/s " +
				"<b>Upload speed:</b> " +
				prettierBytes(window.client.uploadSpeed) +
				"/s " +
				"<b>ETA:</b> " +
				remaining
		);
	...
	downloadZip.addEventListener("click", function (event) {
		let addedFiles = 0;
		var zipFilename =
			path.basename(torrent.name, path.extname(torrent.name)) + ".zip";
		let zip = new JSZip();
